    ...
    for row in &rows {
        ...
        for &col in &ins_cols {
            match col {
                ...
                "start_time" => { let v: i64 = row.get(col); q = q.bind(ms_to_datetime_str(v)); }
                "end_time" => { let v: Option<i64> = row.get(col); q = q.bind(v.map(ms_to_datetime_str)); }
                "duration" => { let v: Option<i64> = row.get(col); q = q.bind(v); }
                _ => {}
            }
        }
...
async fn copy_web(
    ...
    for row in &rows {
        ...
        for &col in &ins_cols {
            match col {
                "domain" => { let v: String = row.get(col); q = q.bind(v); }
                "normalized_domain" => { let v: Option<String> = row.get(col); q = q.bind(v); }
