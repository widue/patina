...
5. 将准备发布所需提交推送到远端。
6. 只有在用户明确进入发布动作时，才推送对应的 `vX.Y.Z` 版本 tag，自动触发 GitHub Actions 的 `Publish Release` 工作流；必要时也可手动触发并输入同一版本号。
...
如果只是把版本号、changelog、发布脚本或 release 说明准备好并推到 `main`，提交信息应避免让人误以为已经发布完成。推荐使用能表达准备状态的提交信息，例如 `chore: prepare vX.Y.Z release`。只有在同一轮明确会推送发布 tag、触发发布工作流时，才使用 `release: vX.Y.Z`。
...
- 准备发布但尚未发布时，提交信息应表达“准备”而不是“已发布”；真正发布提交才使用 `release: vX.Y.Z`
